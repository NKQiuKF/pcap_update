...
from addr import addr2int
from addr import make_mask
...
CHECK_CONNECTION_URL = "https://www.baidu.com"
...
SCAN_PERIOD = 600  #scan local hosts after each SCAN_PERIOD seconds

# Change configure file name to sensor.conf by wangzhi on 2016-03-13
#CONFIG_FILE = os.path.join(ROOT_DIR, "maltrail.conf")
CONFIG_FILE = os.path.join(ROOT_DIR, "sensor.conf")
...
SUSPICIOUS_HTTP_REQUEST_REGEXES = (
    ...
    ("potential remote code execution", r"\$_(REQUEST|GET|POST)\[|xp_cmdshell|\bping(\.exe)? -[nc] \d+|timeout(\.exe)? /T|cmd\.exe|/bin/bash|2>&1|\b(cat|ls) /|nc -l -p \d+|>\s*/dev/null"),
...
WHITELIST_UA_KEYWORDS = ("AntiVir-NGUpd", "TMSPS", "AVGSETUP", "SDDS", "Sophos", "internal dummy connection")
...
def read_config(config_file):
    ...
    #-------------------------------
    # No http server for sensor 
    # by Wang Zhi on 2016-03-13
    #-------------------------------
