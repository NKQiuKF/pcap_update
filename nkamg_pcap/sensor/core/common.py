...
from addr import addr2int, int2addr
from settings import CHECK_CONNECTION_URL, CDN_RANGES, NAME, TIMEOUT
from settings import STATIC_IPCAT_LOOKUPS, TRAILS_FILE, WHITELIST, WORST_ASNS
...
def retrieve_content(url, data=None):
    ...
    try:
        req = urllib2.Request(
            ...
            {"User-agent": NAME, "Accept-encoding": "gzip, deflate"})
        ...
        for name in STATIC_IPCAT_LOOKUPS:
            for value in STATIC_IPCAT_LOOKUPS[name]:
                if "-" in value:
                    ...
                    start_int, end_int = addr2int(start), addr2int(end)
                    current = start_int
                    while start_int <= current <= end_int:
                        _ipcat_cache[int2addr(current)] = name
                        current += 1
                else:
...
def check_sudo():
    """
    Check for sudo/Administrator privileges for sensor.py
    """
