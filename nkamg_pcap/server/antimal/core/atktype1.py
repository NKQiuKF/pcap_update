...
class Atktype1(BaseModel):
    __tablename__ = 'atktype1'
    ...
    def __repr__(self):
        return '<atktype1: %r %r>' % (self.id, self.date)
    ...
    def check(date):
        ...
        query = Session.query(Atktype1)
        tmp = query.filter(Atktype1.date == date).first()
        Session.close()
        if(tmp != None):
            return 2
        return 1
    ...
    def add(date,count=0):
        ...
        '''
        ret = 1
        ret = Atktype1.check(date)
        if(ret != 1):
            return ret
        ftime = datetime.now()
        ltime = datetime.now()
        cur = Atktype1(date,count,ftime,ltime)
    ...
    def updateatktype1(Date_DICT = {}):
        ...
        try:
            ...
            for date in Date_DICT.keys():
                ...
                ret = Atktype1.check(date)
                if(ret == 1):
                    Atktype1.add(date,Date_DICT[date])
                    continue
                elif(ret == 2):
                    curCount = Session.query(Atktype1.count).filter(Atktype1.date == date).first()
                    if(curCount == None):
                        continue
                    else:
                        count = int(curCount[0]) + Date_DICT[date]
                        Session.query(Atktype1).filter(Atktype1.date == date).update({'count':count, 'ltime':datetime.now()})
                        Session.commit()
                else:
                    continue
        except:
