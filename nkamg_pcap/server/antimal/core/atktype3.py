...
class Atktype3(BaseModel):
    __tablename__ = 'atktype3'
    ...
    def __repr__(self):
        return '<atktype3: %r %r>' % (self.id, self.date)
    ...
    def check(date):
        ...
        query = Session.query(Atktype3)
        tmp = query.filter(Atktype3.date == date).first()
        Session.close()
        if(tmp != None):
            return 2
        return 1
    ...
    def add(date,count=0):
        ...
        '''
        ret = 1
        ret = Atktype3.check(date)
        if(ret != 1):
            return ret
      
        ftime = datetime.now()
        ltime = datetime.now()
        cur = Atktype3(date,count,ftime,ltime)
    ...
    def updateatktype3(Date_DICT = {}):
        ...
        try:
            ...
            for date in Date_DICT.keys():
                ...
                ret = Atktype3.check(date)
                if(ret == 1):
                    Atktype3.add(date,Date_DICT[date])
                    continue
                elif(ret == 2):
                    curCount = Session.query(Atktype3.count).filter(Atktype3.date == date).first()
                    if(curCount == None):
                        continue
                    else:
                        count = int(curCount[0]) + Date_DICT[date]
                        Session.query(Atktype3).filter(Atktype3.date == date).update({'count':count, 'ltime':datetime.now()})
                        Session.commit()
                else:
                    continue
        except:
