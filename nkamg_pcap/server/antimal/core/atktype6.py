...
class Atktype6(BaseModel):
    __tablename__ = 'atktype6'
    ...
    def __repr__(self):
        return '<atktype6: %r %r>' % (self.id, self.date)
    ...
    def check(date):
        ...
        tmp = query.filter(Atktype6.date == date).first()
        Session.close()
        if(tmp != None):
            return 2
        return 1
    ...
    def add(date,count=0):
        ...
        '''
        ret = 1
        ret = Atktype6.check(date)
        if(ret != 1):
            return ret
        
        ftime = datetime.now()
        ltime = datetime.now()
        cur = Atktype6(date,count,ftime,ltime)
    ...
    def updateatktype6(Date_DICT = {}):
        ...
        try:
            ...
            for date in Date_DICT.keys():
                ...
                ret = Atktype6.check(date)
                if(ret == 1):
                    Atktype6.add(date,Date_DICT[date])
                    continue
                elif(ret == 2):
                    curCount = Session.query(Atktype6.count).filter(Atktype6.date == date).first()
                    if(curCount == None):
                        continue
                    else:
                        count = int(curCount[0]) + Date_DICT[date]
                        Session.query(Atktype6).filter(Atktype6.date == date).update({'count':count, 'ltime':datetime.now()})
                        Session.commit()
                else:
                    continue
        except:
