...
class Outip(BaseModel):
    ...
    def check(name):
        ...
        tmpAgent = query.filter(Outip.name == name).first()
        Session.close()
        if(tmpAgent != None):
            return 2
        return 1
    ...
    def add(name,count=0):
        ...
        ftime = datetime.now()
        ltime = datetime.now()
        curAgent = Outip(name,count,ftime,ltime)
        ...
        try:
            ...
            for name in Name_DICT.keys():
                ...
                if(ret == 1):
                    ...
                    curCount = Session.query(Outip.count).filter(Outip.name == name).first()
                    if(curCount == None):
                        continue
                    else:
                        count = int(curCount[0]) + Name_DICT[name]
                        Session.query(Outipt).filter(Outip.name == name).update({'count':count, 'ltime':datetime.now()})
                        Session.commit()
                else:
                    continue
        except:
