        ...
        try:
            ...
            for date in Date_DICT.keys():
                ...
                if(ret == 1):
                    ...
                    curCount = Session.query(Atktype4.count).filter(Atktype4.date == date).first()
                    if(curCount == None):
                        continue
                    else:
                        count = int(curCount[0]) + Date_DICT[date]
                        Session.query(Atktype4).filter(Atktype4.date == date).update({'count':count, 'ltime':datetime.now()})
                        Session.commit()
                else:
                    continue
        except:
