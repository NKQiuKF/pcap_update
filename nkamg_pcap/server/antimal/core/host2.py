...
class Host2(BaseModel):
    ...
    def check(name):
        ...
        tmpHost = query.filter(Host2.name == name).first()
        Session.close()
        if(tmpHost != None):
            return 2
        return 1
    ...
    def add(name,count=0):
        ...
        ftime = datetime.now()
        ltime = datetime.now()
        curHost = Host2(name,count,ftime,ltime)
        ...
        try:
            ...
            for name in Name_DICT.keys():
                ...
                if(ret == 1):
                    ...
                    curCount = Session.query(Host2.count).filter(Host2.name == name).first()
                    if(curCount == None):
                        continue
                    else:
                        count = int(curCount[0]) + Name_DICT[name]
                        Session.query(Host2).filter(Host2.name == name).update({'count':count, 'ltime':datetime.now()})
                        Session.commit()
                else:
                    continue
        except:
