...
int open_log_file(f_state *s)
{
	...
	if ((s->log_file = fopen(fn, "w")) == NULL)
		{
		print_error(s, fn, strerror(errno));
		fatal_error(s, "Can't open log file");
		}

	log_msg(s, "#separator \\x09\n");
	log_msg(s, "#Foremost started at %s\n", get_start_time(s));
	log_msg(s, "#Invocation: %s\n", get_invocation(s));
	log_msg(s, "#Output directory: %s\n", get_output_directory(s));
	log_msg(s, "#\n");
	log_msg(s, "#\n");
	log_msg(s, "#fields	\tts\torig_h\torig_p\tresp_h\tresp_p\tfilename\tbytes\tinput\ts_offset\tcomment\n");
	log_msg(s, "#types\ttime\taddr\tport\taddr\tport\tstring\tcount\tstring\tcount\tstring\n");

	return FALSE;
}

int close_log_file(f_state *s)
{
	if (fclose(s->log_file))
		{
		print_error(s, LOG_FILE_NAME, strerror(errno));
		return TRUE;
		}
...
void log_msg(f_state *s, char *format, ...)
{
	va_list argp;
	va_start(argp, format);

	vfprintf(s->log_file, format, argp);
	va_end(argp);
...
void set_input_file_type(f_state *s, enum INPUT_FILE_TYPE type)
{
	s->input_file_type = type;
}

enum INPUT_FILE_TYPE get_input_file_type(f_state *s)
{
	return s->input_file_type;
}
