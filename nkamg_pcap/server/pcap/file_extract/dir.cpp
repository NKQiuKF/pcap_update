...
int create_output_directory(f_state *s)
{
	...
	if(!s->output_directory){
		if( make_new_directory(s, DEFAULT_OUTPUT_DIRECTORY) ||
...
int write_to_disk(f_state *s, s_spec *needle, u_int64_t len, unsigned char *buf, u_int64_t t_offset, void *con)
{
	...
	connection_t *conn = (connection_t*)con;
	if(con){
		...
		if (iter != g_sessions.end())
			{
			ci = iter->second;
			audit_msg(s, "%s:%d->%s:%d", inet_ntoa(*(in_addr*) &conn->ip_src),
					ntohs(conn->port_src), inet_ntoa(*(in_addr*) &conn->ip_dst),
					ntohs(conn->port_dst));

			//ts	id.orig_h	id.orig_p	id.resp_h	id.resp_p
			log_msg(s, "%d.%06d\t%s\t%d\t%s\t%d\t",
					ci->_ts.tv_sec, ci->_ts.tv_usec,
					inet_ntoa(*(in_addr*) &conn->ip_src), ntohs(conn->port_src),
					inet_ntoa(*(in_addr*) &conn->ip_dst), ntohs(conn->port_dst));
		}
	}
	...
	snprintf(fn,
			 MAX_STRING_LENGTH,
			 "%s/%s/%s_%d_%s_%d_%0*llu.%s",
			 s->output_directory,
			 needle->suffix,
			 inet_ntoa(*(in_addr*)&conn->ip_src),
			 ntohs(conn->port_src),
			 inet_ntoa(*(in_addr*)&conn->ip_dst),
			 ntohs(conn->port_dst),
			 8,
			 block,
			 needle->suffix);
	...
	if (i == 1) {
		//0:	00000004_6.zip 	       1 KB 	       2307 	  (Header dump)
      audit_msg(s,"%d:\t%08llu.%s \t %10s \t %10llu \t %s",
         ...
         t_offset,
         needle->comment);
      //filename	bytes	input	s_offset	comment
      //00000004_6.zip	3275	1.pcap	2307	(Header dump)
      log_msg(s,"%08llu.%s\t%d\t%s\t%llu\t%s\n",
         block,
         needle->suffix,
         len,
		 s->input_file,
      ...
      audit_msg(s,"%d:\t%08llu_%d.%s \t %10s \t %10llu \t %s",
         ...
         t_offset,
         needle->comment);
      log_msg(s,"%08llu_%d.%s\t%d\t%s\t%llu\t%s\n",
         block,
         i - 1,
         needle->suffix,
         len,
