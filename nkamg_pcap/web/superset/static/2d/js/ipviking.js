...
(function(window) {
    ...
    */
    ...
    var settings = {
        ...
        // Layout setti/media/qkf/EAGET/antimalghzzzzngs
        linkAnchor: false,
        linkSiblings: false,
        ...
        // Websocket settings
        wsHost: "ws://127.0.0.1:8080/", //HOST_IP
    ...
    (function() {
        var toggles = d3.selectAll(".switch");
        var data = toggles[0]
            .map(function(elt) { 
            	return {
                                         current : elt.getAttribute("data-default") ,
                                         next : elt.getAttribute("data-second")
                                     }
                                  });
            	//return {default:d3.select(elt.getAttribute("data-default")); },second:d3.select(elt.getAttribute("data-second")); };});
    ...
    var projection = d3.geo.mercator()
        .scale(width / 8.5)
        .translate([width / 2 - 50, height / 1.7])
        .rotate([-499.62, 0, 0]);
    ...
    function spanWrap(content, classes, style) {
        // Returns the content wrapped in the span
        return '<span class="' + (classes ? classes.join(" ") : "") + '"' + (style ? ' style="'+style+'"' : '') + '>' +
            content + '</span>'
    }
    ...
    function flagPath(iso) {
        ...
        if (iso === "O1") {
            return "./static/2d/images/militarywhite.svg";
        } else {
    ...
    var nodeModel = {
        ...
        pushAttack: function(attack) {
             ...
             */
            // Clear out old nodes
            while (this.get(this.ATTACKS).length > 20) {
    ...
    var circleScale = function(x) { var num = Math.ceil(1.4 * logScale(x)); return num>5?5:num; };
    ...
    var _ = (function() {
        ...
        canvas.on("click", function() {
            if (previous) {
            	d3.select("#cityname").text(previous[1]);
                statsManager.attacked.draw(previous[0], previous[1]);
            }else{
            	d3.select("#info").style({display:"none"});
            }
        });
    ...
    var statsManager = {
        ...
        stats: [
            new Stats({
                ...
                insert: function(incoming, state) {
                    ...
                    if (state.has(incoming.country2)) {
                    	  var country2 = state.get(incoming.country2);
                    } else {
                    	  country2 = d3.map();
                        state.set(incoming.country2, country2);
                    }
                    if (country2.has(incoming.dport)) {
                        country2.set(incoming.dport,
                                  country2.get(incoming.dport) + 1);
                    } else {
                        country2.set(incoming.dport, 1);
                    }
                },
            ...
            new Stats({
                ...
                insert: function(incoming, state) {
                    ...
                    if (state.has(incoming.country2)) {
                        ...
                        state.set(incoming.country2, 1);
								...
								chartWidth : 160,
								chartHeight : 160,
								chartRadius : function(){
									return Math.min(this.chartWidth, this.chartHeight) / 2;
									},
								chartSvg: function(){
									var svg1 = d3.select('#right-chart-container svg g');
									if(!svg1[0][0]){
										
										var svg2 = d3.select("#right-chart-container").append("svg")
                ...
                redraw: function() {
                    ...
                    var test = this.chartSvg();
										test.selectAll('g').remove();
										...
										var labelArc = d3.svg.arc()
										    .outerRadius(this.chartRadius() - 40)
										    .innerRadius(this.chartRadius() - 40);
									  ...
									  g1.append("text")
									      ...
									      .attr("dy", ".35em")
									      .text(function(d) { 
									      	return countryModel.getByIso2(d.data.key).country; 
									      	});
                }
            }),
                    ...
                    var cols = rows.selectAll("td")
                        .data(function(d) {
                            return [
                                ...
                                //        countryModel.getByIso2(d.country)?countryModel.getByIso2(d.country).country:d.country,
                                //    ["location", "overflow"]),
                                //d.md5,
                                //spanWrap(
                                //    (d.city2 === "" ? "unknown" : d.city2) + ", " +
                                //    countryModel.getByIso2(d.country2)?countryModel.getByIso2(d.country2).country:d.country2,
                                //    ["location", "overflow", "numeric"]),
                                //spanWrap(d.service || "unknown",
                                //              ["service", "overflow"], "color:"+colorizer(d.dport)),
        ...
        attacked: {
                ...
                state: d3.map(),
                 ...
                 insert: function(incoming) {
                     this.updated = incoming.dport;
                     if (!(incoming.country2 in this.state)) {
                			  this.state[incoming.country2] = {};
            				 }
                     if (!(incoming.city2 in this.state[incoming.country2])) {
                			  this.state[incoming.country2][incoming.city2] = d3.map();
            				 }
            				 var _state = this.state[incoming.country2][incoming.city2];
                     if (_state.has(incoming.dport)) {
                         _state.set(incoming.dport,
                                        _state.get(incoming.dport) + 1);
                     } else {
                         _state.set(incoming.dport, 1);
                     }
                 },

                 draw: function(country, city) {
                 		 var countryData = this.state[country];
                 		 if(!countryData) return;
                 		 var cityData = countryData[city];
                 		 if(!cityData) return;
                     var data = cityData.entries()
                         .sort(function(d1, d2) { return d2.value - d1.value; })
                         .slice(0, settings.portTableRows),
                         updated = this.updated;

                     var rows = d3.select("#info-data").selectAll("tr.row")
                         // XXX - I'm not sure why d is undef the first time through.
                         .data(data, function(d, i) { return d ? d.key : i; });
                     rows.enter()
                         .append("tr")
    ...
    function start(loc, psk) {
        var webSocket = new WebSocket(loc || settings.wsHost, 'echo-protocol');
        ...
        webSocket.onclose = function() {
            ...
            d3.select("#events-data").selectAll("tr.row").remove(); 
            d3.select("#events-data").append("tr").attr('class', 'row').html("<td colspan='7'><img src='../../static/2d/images/loading.gif' style='margin-top: 6px;'/>&nbsp;<span style='display: inline-block; height: 25px; vertical-align: middle;'>Loading...</span></td>");
    ...
    queue()
        .defer(d3.json, "../../static/2d/data/readme-world.json")
        .defer(d3.tsv, "../../static/2d/data/port-names.tsv")
        .defer(d3.csv, "../../static/2d/data/country-codes.csv")
        .await(function (error, world, rawPorts, countryCodes) {
            ...
            svg.append("g")
                ...
                .attr('stroke', '#444444')
                .attr('stroke-width', 1)
                .attr('title', "teststest")
                .on('mouseover', function(data){
                	d3.select(this).attr('stroke', '#4bffff');
                })
                .on('mouseout', function(data) {
                	d3.select(this).attr('stroke', '#444444');
                })
                .on('click', function(data) {
                	
                    d3.select("#info-data").selectAll("tr.row").remove();
                	  var country = d3.select(this).attr('id');
                	
                	
                	  if(!statsManager.stats[0].state.get(country))
                	    return;
                    var data = statsManager.stats[0].state.get(country).entries()
                        .sort(function(d1, d2) { return d2.value - d1.value; })
                        .slice(0, settings.topTableRows);
                         ...
                         .data(function(d) {
                             return [
                                '<div class="bar" style="width: ' +
                                 ...
                                 '<span class="port-circle" style="color:' + colorizer(d.key) +
                                     '">●</span>',
                                 spanWrap(ports[d.key] ? ports[d.key] : "unknown", ["test"], "color:"+colorizer(d.key)),
                                 spanWrap(d.key, ["numeric"])
			     ]; });
                     //function(d) { return d; });
                     cols.enter().append("td");
