...
from superset import (
    ...
    sm, sql_lab, results_backend, security, socketio, emit,
    csrf)
...
if config.get('ENABLE_ACCESS_REQUEST'):
    ...
    class AccessRequestsModelView(SupersetModelView,mDeleteMixin):
...
class R(BaseSupersetView):
    ...
    def shortner(self):
        ...
        db.session.add(obj)
        db.session.commit()
        return ("http://{request.headers[Host]}/r/{obj.id}".format(request=request, obj=obj))
...
from superset import csrf,emit,socketio
import sqlite3
# text_factory = str

superset_path = os.path.dirname(os.path.abspath(__file__))
db_path = os.path.dirname(os.path.abspath(__file__)).split('/')[:-1]
db_file = ('/').join(db_path)+'/.superset/'

class Map_realtime(BaseSupersetView):
    default_view = 'world3d'

    @expose('/world3d/')
    @has_access
    def world3d(self):
        return self.render_template('world_3d.html')

    @expose('/china2d/')
    @has_access
    def china2d(self):
        return self.render_template('china_2d.html')
...
appbuilder.add_view(
    ...
    category="Map",
    category_label=__("地图")
    # category_label=__("Map")
)
appbuilder.add_link(
    'china_2d',
    label=_("中国2D地图"),
    href="/map_realtime/china2d",
    icon="fa-heart",
    category="Map"
)

#class EQP(BaseSupersetView):
#    default_view = 'eqp'
#
#    @expose('/eqp')
#    @has_access
#    def eqp(self):
#        return self.render_template('eqp.html')
#
#    text_factory = str
#
#
...
class atk_records(BaseSupersetView):
    default_view = 'atk_records'

    @expose('/atk_records')
    @has_access
    def atk_records(self):
        return self.render_template('atk_records.html')
...
class history(BaseSupersetView):
...
class status(BaseSupersetView):
    default_view = 'status_'
...
import shutil
import hashlib
webpath = '/'.join(os.path.abspath(__file__).split('/')[:-3])
UPLOAD_FOLDER = '{web}/file'.format(web=webpath)
TMP_FOLDER = '{web}/superset/tmp'.format(web=webpath)

# print(os.path.join(app.config['UPLOAD_FOLDER']))
db_path = db_file
db_file = db_file+'/superset.db'
...
def loading():
    ...
    flag = 0
    while 1:  
        result = db.session.query(models.Dashboard).filter_by(slug=filename).first()
        if result:
            return filename
        flag = flag+1
        time.sleep(1)
...
suffix={'application/vnd.tcpdump.pcap':'.pcap'}
def allowed_file(filetype):
    if filetype in suffix.keys():
        return True
    else:
        return False


class Upload(BaseSupersetView):
    default_view = 'upload'
    ...
    def upload(self):
        if request.method == 'POST':
            if os.path.exists(TMP_FOLDER):
                ...
                os.mkdir(TMP_FOLDER)
            uploaded_pcapfiles = request.files.getlist("pcapfile")
            if uploaded_pcapfiles[0]:
                for fl in uploaded_pcapfiles:
                    check = 0
                    if allowed_file(fl.content_type):
                        print(fl.content_type)
                        fl.save(os.path.join(TMP_FOLDER,fl.filename))
                        filehash = sha256(os.path.join(TMP_FOLDER,fl.filename))
                        try:
                            check = fl.filename
                            results = db.session.query(models.Dashboard).filter_by(slug=filehash).limit(1).one()
                            if results:
                                os.remove(os.path.join(TMP_FOLDER,fl.filename))
                                print('redirect',1111111111111111111)
                                return redirect('/superset/dashboard/{SHA256}/?preselect_filters='.format(SHA256=filehash)+'{}')
                        except Exception,e:
                            shutil.move(os.path.join(TMP_FOLDER,fl.filename),UPLOAD_FOLDER)
                            if check != 0:
                                return self.render_template('upload_success.html',filename=filehash)
...
from superset.views.lib.serverInfo import get_server_info
from superset.views.lib.sql.agent import Agent
from superset.views.lib.sql.agent2 import Agent2
from superset.views.lib.sql.host import Host
from superset.views.lib.sql.host2 import Host2
from superset.views.lib.sql.domain import Domain
from superset.views.lib.sql.domain2 import Domain2
from superset.views.lib.sql.outip import Outip
from superset.views.lib.sql.outip2 import Outip2
from superset.views.lib.sql.atktype1 import Atktype1
from superset.views.lib.sql.atktype2 import Atktype2
from superset.views.lib.sql.atktype3 import Atktype3
from superset.views.lib.sql.atktype4 import Atktype4
from superset.views.lib.sql.atktype5 import Atktype5
from superset.views.lib.sql.atktype6 import Atktype6
from superset.views.lib.sql.eqp import Eqp

@app.route('/tmp/server_info')
def tem_server_info():
    """
    index  Disk CPU Memory
    """
    return jsonify(get_server_info())


@app.route('/tmp/refresh_eqp', methods=['GET'])
def refresh_eqp():
    """
    show all equipments
    """

    def get_eqp_list(eqp):
        data.append({
            "name": eqp.name,
            "department": eqp.department,
            "id": eqp.id,
            "ip": eqp.ip,
            "mac": eqp.mac,
            "os": eqp.os,
            "computer": eqp.computer,
            "authority": eqp.authority,
            "first_time": eqp.first_time.strftime('%Y-%m-%d %H:%M:%S'),
            "last_time": eqp.last_time.strftime('%Y-%m-%d %H:%M:%S')})

    data = []
    result = {'status': 1}
    try:
        list_eqp = Eqp.get_all_eqp()
        map(get_eqp_list, list_eqp)
        result['data'] = data
...
@socketio.on('eqp_auth_click')
def change_eqp_auth(msg):
    Eqp.update_auth(msg)
...
def deal_tmp_pie():
    ...
    ret['data']['agent2'] = Agent2.info(5)['data']
    ret['data']['host'] = Host.info(5)['data']
    ret['data']['host2'] = Host2.info(5)['data']
    ret['data']['domain'] = Domain.info(5)['data']
    ret['data']['domain2'] = Domain2.info(5)['data']
    ret['data']['outip'] = Outip.info(5)['data']
    ret['data']['outip2'] = Outip2.info(5)['data']
    return jsonify(ret)
...
def deal_tmp_line():
    ...
    """
    ret = {'data': {}}
    ret['data']['atktype1'] = Atktype1.info(7)['data']
    ret['data']['atktype2'] = Atktype2.info(7)['data']
    ret['data']['atktype3'] = Atktype3.info(7)['data']
    ret['data']['atktype4'] = Atktype4.info(7)['data']
    ret['data']['atktype5'] = Atktype5.info(7)['data']
    ret['data']['atktype6'] = Atktype6.info(7)['data']
    return jsonify(ret)
...
@app.route("/heatmap")
def get_data():
    def get_atk_type(num):
        atk_dict = ['malware', 'hacker', 'spammer', 'crawler', 'compromised']
        return atk_dict[num]
    ...
    with open('{sp}/heat_input/geojson/china_provinces_en.json'.format(sp=superset_path)) as data_file:
        provinces_json = json.load(data_file)

    get_atk_info3 = pd.read_csv('{sp}/heat_input/heat_map3.csv'.format(sp=superset_path))
    get_atk_info1 = pd.read_csv('{sp}/heat_input/heat_map1.csv'.format(sp=superset_path))
    get_atk_info2 = pd.read_csv('{sp}/heat_input/heat_map2.csv'.format(sp=superset_path))

    df = get_atk_info3.merge(get_atk_info1, how='left', on='src_ip')
    df = df.merge(get_atk_info2, how='left', on='src_ip')
    # Get n_samples records
    df = df[df['src_longitude'] != 0]

    top_10_atk = {
        'malwarepatrol.net': 'malwarepatrol.net',
        'malwaredomains.com': 'malwaredomains.com',
        'bambenekconsulting.com': 'bambenekconsulting.com',
        'alienvault.com (+turris.cz)': 'alienvault.com',
        'openphish.com': 'openphish.com',
    ...
    df['top_10_trail'] = df['gender'].apply(
        lambda trail_ref: top_10_atk[trail_ref] if (trail_ref in top_10_atk) else 'Other')
    df['atk_type'] = df['atkid'].apply(lambda age: get_atk_type(age))
    df['src_location'] = df.apply(
        lambda row: get_location(row['src_longitude'], row['src_latitude'], provinces_json), axis=1)
    df['dst_location'] = df.apply(
        lambda row: get_location(row['dst_longitude'], row['dst_latitude'], provinces_json), axis=1)
    ...
    df_cols = [
        ...
        'src_location',
        'dst_location']
    df_clean = df[df_cols].dropna()
    return df_clean.to_json(orient='records')


@app.route('/tmp/history')
def get_history_records():
#    superset_path = os.path.dirname(os.path.abspath(__file__))
    history_path = '{sp}/history.csv'.format(sp=superset_path)
    if not os.path.exists(history_path):
        ...
        df_columns = [
            ...
            'reference',
            'info']
        history = pd.read_csv(history_path, names=df_columns)
        return history.to_json(orient='records')

# from app import views  # 这句不能动
