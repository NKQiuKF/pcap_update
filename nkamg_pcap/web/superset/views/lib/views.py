...
class UserModelView(ModelView):
    ...
    def userinfoedit(self, item):
        return redirect(
            url_for(
                self.appbuilder.sm.userinfoeditview.__name__ +
                '.this_form_get'))
...
class RoleModelView(ModelView):
    ...
    @action("Copy Role", lazy_gettext('Copy Role'), lazy_gettext(
        'Copy the selected roles?'), icon='fa-copy', single=False)
...
class AuthDBView(AuthView):
    ...
    def login(self):
        ...
        if form.validate_on_submit():
            ...
            return redirect('/superset/dashboard/6/?preselect_filters=%7B%7D')
#            return redirect(self.appbuilder.get_url_for_index)
...
class AuthOAuthView(AuthView):
    ...
    def oauth_authorized(self, provider):
        ...
        try:
            ...
            log.debug(
                "User info retrieved from {0}: {1}".format(
                    provider, userinfo))
        except Exception as e:
