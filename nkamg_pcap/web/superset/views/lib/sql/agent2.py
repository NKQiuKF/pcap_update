...
class Agent2(BaseModel):
    ...
    def __repr__(self):
        return '<agent2: %r %r>' % (self.id, self.name)
    ...
    def info(num):
        ...
        pureData = query.filter().order_by(desc(Agent2.count)).limit(num).all()
    ...
    def check(name):
        ...
        query = Session.query(Agent2)
        tmpAgent2 = query.filter(Agent2.name == name).first()
        Session.close()
        if(tmpAgent2 is not None):
            return 2
        return 1
    ...
    def add(name, count=0):
        ...
        ftime = datetime.now()
        ltime = datetime.now()
        curAgent2 = Agent2(name, count, ftime, ltime)

        Session = scoped_session(sessionmaker(autoflush=True, bind=engine))
        Session.add(curAgent2)
        Session.commit()
        Session.close()
        return ret

    @staticmethod
    def updateAgent2(Name_DICT={}):
        '''
        :param Name_DICT:
            ...
            for name in Name_DICT.keys():
                ...
                if(ret == 1):
                    ...
                    curCount = Session.query(
                        Agent2.count).filter(
                        Agent2.name == name).first()
                    if(curCount is None):
                        continue
                    else:
                        count = int(curCount[0]) + Name_DICT[name]
                        Session.query(Agent2).filter(Agent2.name == name).update(
