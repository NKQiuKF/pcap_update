...
class Outip(BaseModel):
    ...
    def __repr__(self):
        return '<outip: %r %r>' % (self.id, self.name)

    @staticmethod
    def info(num):
        ret = {}
        ret['data'] = []
        query = session.query(Outip)
        pureData = query.filter().order_by(desc(Outip.count)).limit(num).all()
    ...
    def check(name):
        ...
        if(name == ''):
            return 3
        query = session.query(Outip)
        tmpOutip = query.filter(Outip.name == name).first()
        if(tmpOutip is not None):
            return 2
        return 1
    ...
    def add(name, count=0):
        ...
        ftime = datetime.now()
        ltime = datetime.now()
        curOutip = Outip(name, count, ftime, ltime)

        Session = scoped_session(sessionmaker(autoflush=True, bind=engine))
        Session.add(curOutip)
        Session.commit()
        Session.close()
        return ret
