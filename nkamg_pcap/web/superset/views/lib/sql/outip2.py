...
class Outip2(BaseModel):
    ...
    def check(name):
        ...
        if(name == ''):
            return 3
        query = session.query(outip2)
        tmpOutip2 = query.filter(Outip2.name == name).first()
        if(tmpOutip2 is not None):
            return 2
        return 1
    ...
    def add(name, count=0):
        ...
        ftime = datetime.now()
        ltime = datetime.now()
        curOutip2 = Outip2(name, count, ftime, ltime)

        Session = scoped_session(sessionmaker(autoflush=True, bind=engine))
        Session.add(curOutip2)
        Session.commit()
        Session.close()
        return ret

    @staticmethod
    def updateOutip2(Name_DICT={}):
        '''
        :param Name_DICT:
            ...
            for name in Name_DICT.keys():
                ...
                if(ret == 1):
                    ...
                    curCount = session.query(
                        Outip2.count).filter(
                        Outip2.name == name).first()
                    if(curCount is None):
                        continue
                    else:
                        count = int(curCount[0]) + Name_DICT[name]
                        session.query(Outip2).filter(Outip2.name == name).update(
